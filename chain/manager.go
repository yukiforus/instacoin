...
type ManagerStore interface {
	...
	Checkpoint(index instacoin.ChainIndex) (consensus.Checkpoint, error)
	Header(index instacoin.ChainIndex) (instacoin.BlockHeader, error)

	ExtendBest(index instacoin.ChainIndex) error
	RewindBest() error
	BestIndex(height uint64) (instacoin.ChainIndex, error)
...
func (m *Manager) Tip() instacoin.ChainIndex {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.vc.Index
}
...
func (m *Manager) Block(index instacoin.ChainIndex) (instacoin.Block, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// index.
func (m *Manager) ValidationContext(index instacoin.ChainIndex) (consensus.ValidationContext, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	c, err := m.store.Checkpoint(index)
...
func (m *Manager) History() ([]instacoin.ChainIndex, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	var history []instacoin.ChainIndex
	for {
...
func (m *Manager) HeadersForHistory(headers []instacoin.BlockHeader, history []instacoin.ChainIndex) ([]instacoin.BlockHeader, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) AddHeaders(headers []instacoin.BlockHeader) (*consensus.ScratchChain, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(headers) == 0 {
		return nil, nil
...
// reorg.
func (m *Manager) AddBlocks(blocks []instacoin.Block) (*consensus.ScratchChain, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(blocks) == 0 {
		return nil, nil
...
func (m *Manager) AddTipBlock(b instacoin.Block) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) applyTip(index instacoin.ChainIndex) error {
	c, err := m.store.Checkpoint(index)
	if err != nil {
...
func (m *Manager) reorgTo(sc *consensus.ScratchChain) error {
	...
	// current best chain
	var rebase []instacoin.ChainIndex
...
// updated accordingly.
func (m *Manager) AddSubscriber(s Subscriber, tip instacoin.ChainIndex) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.vc.Index != tip {
