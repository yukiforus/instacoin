...
func (s *Syncer) handleMsgGetHeaders(p *Peer, msg *MsgGetHeaders) Message {
	...
	headers, err := s.cm.HeadersForHistory(make([]instacoin.BlockHeader, 2000), msg.History)
	if err != nil {
...
func (s *Syncer) syncLoop() {
	...
	seen := make(map[instacoin.ChainIndex]bool)
	for s.err == nil {
...
func (s *Syncer) syncToTarget(target instacoin.ChainIndex, targetPeer *Peer) {
	const blocksPerRequest = 16

	// helper functions for calling RPCs
	getHeaders := func(p *Peer, history []instacoin.ChainIndex) ([]instacoin.BlockHeader, error) {
		var resp MsgHeaders
	...
	getBlocks := func(p *Peer, blocks []instacoin.ChainIndex) ([]instacoin.Block, error) {
	...
	for {
		...
		if errors.Is(err, chain.ErrUnknownIndex) {
			...
			history = []instacoin.ChainIndex{headers[len(headers)-1].Index()}
			continue
		}
		...
		history = []instacoin.ChainIndex{sc.ValidTip()}
	}
}
...
func NewSyncer(addr string, genesisID instacoin.BlockID, cm *chain.Manager, tp *txpool.Pool) (*Syncer, error) {
