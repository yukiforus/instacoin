...
func TestBlockRewardValue(t *testing.T) {
	reward := func(height uint64) instacoin.Currency {
		return (&ValidationContext{Index: instacoin.ChainIndex{Height: height - 1}}).BlockReward()
	}
...
func TestAccumulator(t *testing.T) {
	randAddr := func() (addr instacoin.Address) {
		rand.Read(addr[:])
		return
	}
	randAmount := func() instacoin.Currency {
		var b [16]byte
		rand.Read(b[:])
		return instacoin.NewCurrency(
			binary.LittleEndian.Uint64(b[:8]),
			binary.LittleEndian.Uint64(b[8:]),
		)
	}
	containsOutput := func(sa StateAccumulator, o instacoin.Output, flags uint64) bool {
		return sa.containsObject(outputStateObject(o, flags))
	}

	b := genesisWithBeneficiaries([]instacoin.Beneficiary{
	...
	b = instacoin.Block{
		...
		Transactions: []instacoin.Transaction{txn},
	}
	...
	outputsWithRevert := append([]instacoin.Output(nil), origOutputs...)
	for i := range outputsWithRevert {
		outputsWithRevert[i].MerkleProof = append([]instacoin.Hash256(nil), outputsWithRevert[i].MerkleProof...)
	...
	b = instacoin.Block{
		...
		Transactions: []instacoin.Transaction{parentTxn, childTxn},
	}
...
func TestUpdateExistingObjects(t *testing.T) {
	outputs := make([]instacoin.Output, 8)
	...
	addOutput := func(o instacoin.Output, flags uint64) {
...
func TestMultiproof(t *testing.T) {
	outputs := make([]instacoin.Output, 8)
	leaves := make([]instacoin.Hash256, len(outputs))
	for i := range outputs {
	...
	outputs[0].MerkleProof = []instacoin.Hash256{leaves[1], node23, node47}
	outputs[1].MerkleProof = []instacoin.Hash256{leaves[0], node23, node47}
	outputs[2].MerkleProof = []instacoin.Hash256{leaves[3], node01, node47}
	outputs[3].MerkleProof = []instacoin.Hash256{leaves[2], node01, node47}
	outputs[4].MerkleProof = []instacoin.Hash256{leaves[5], node67, node03}
	outputs[5].MerkleProof = []instacoin.Hash256{leaves[4], node67, node03}
	outputs[6].MerkleProof = []instacoin.Hash256{leaves[7], node45, node03}
	outputs[7].MerkleProof = []instacoin.Hash256{leaves[6], node45, node03}

	tests := []struct {
		inputs []int
		proof  []instacoin.Hash256
	}{
		{
			inputs: []int{0},
			proof:  []instacoin.Hash256{leaves[1], node23, node47},
		},
		{
			inputs: []int{1, 2, 3},
			proof:  []instacoin.Hash256{leaves[0], node47},
		},
		{
			inputs: []int{7, 6, 0, 2, 3},
			proof:  []instacoin.Hash256{leaves[1], node45},
		},
		{
	...
	for _, test := range tests {
		txns := []instacoin.Transaction{{Inputs: make([]instacoin.Input, len(test.inputs))}}
		...
		for _, txn := range txns {
			for i := range txn.Inputs {
				txn.Inputs[i].Parent.MerkleProof = make([]instacoin.Hash256, len(txn.Inputs[i].Parent.MerkleProof))
			}
		}
...
func BenchmarkApplyBlock(b *testing.B) {
	block := instacoin.Block{
		Transactions: []instacoin.Transaction{{
			...
			Outputs: make([]instacoin.Beneficiary, 1000),
		}},
	}
	for i := 0; i < b.N; i++ {
...
func BenchmarkUpdateExistingObjects(b *testing.B) {
	outputs := make([]instacoin.Output, 1000)
	objects := make([]stateObject, len(outputs))
	for i := range outputs {
		objects[i] = outputStateObject(outputs[i], 0)
	}
	...
	proofs := make([][]instacoin.Hash256, len(outputs))
	for i := range proofs {
		proofs[i] = append([]instacoin.Hash256(nil), outputs[i].MerkleProof...)
	}
