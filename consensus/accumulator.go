...
func merkleNodeHash(left, right instacoin.Hash256) instacoin.Hash256 {
	buf := make([]byte, 65)
	buf[0] = nodeHashPrefix
	copy(buf[1:], left[:])
	copy(buf[33:], right[:])
	return instacoin.HashBytes(buf)
}

func merkleProofRoot(leafHash instacoin.Hash256, leafIndex uint64, proof []instacoin.Hash256) instacoin.Hash256 {
	root := leafHash
	for i, h := range proof {
		if leafIndex&(1<<i) == 0 {
...
type stateObject struct {
	objHash   instacoin.Hash256
	leafIndex uint64
	flags     uint64
	proof     []instacoin.Hash256
}

func (so stateObject) leafHash() instacoin.Hash256 {
...
func (so stateObject) proofRoot() instacoin.Hash256 {
	return merkleProofRoot(so.leafHash(), so.leafIndex, so.proof)
}

func (so stateObject) truncatedProofRoot(n int) instacoin.Hash256 {
	return merkleProofRoot(so.leafHash(), so.leafIndex, so.proof[:n])
}

func outputStateObject(o instacoin.Output, flags uint64) stateObject {
...
func objectsByTree(txns []instacoin.Transaction) [64][]stateObject {
	...
	for _, txn := range txns {
		for _, in := range txn.Inputs {
			if in.Parent.LeafIndex != instacoin.EphemeralLeafIndex {
				addObject(outputStateObject(in.Parent, 0))
			}
		}
	}
...
func updateProof(proof []instacoin.Hash256, leafIndex uint64, updated *[64][]stateObject) {
...
// accumulator.
func (sa *StateAccumulator) ContainsUnspentOutput(o instacoin.Output) bool {
	return sa.containsObject(outputStateObject(o, 0))
}
...
// tree.
func (sa *StateAccumulator) addNewObjects(objects []stateObject) [64][]instacoin.Hash256 {
	initialLeaves := sa.NumLeaves
	var treeGrowth [64][]instacoin.Hash256
	for i := range objects {
		objects[i].leafIndex = sa.NumLeaves
		// TODO: preallocate this more accurately
		objects[i].proof = make([]instacoin.Hash256, 0, trailingOnes(sa.NumLeaves))
...
func (sa *StateAccumulator) updateExistingObjects(objects []stateObject) [64][]stateObject {
	var recompute func(i, j uint64, objects []stateObject) instacoin.Hash256
	recompute = func(i, j uint64, objects []stateObject) instacoin.Hash256 {
		...
		var leftRoot, rightRoot instacoin.Hash256
		if len(left) == 0 {
...
func MultiproofSize(txns []instacoin.Transaction) int {
...
func ComputeMultiproof(txns []instacoin.Transaction) (proof []instacoin.Hash256) {
...
// correct size.
func ExpandMultiproof(txns []instacoin.Transaction, proof []instacoin.Hash256) {
	var expand func(i, j uint64, objects []stateObject) instacoin.Hash256
	expand = func(i, j uint64, objects []stateObject) instacoin.Hash256 {
		height := bits.TrailingZeros64(j - i)
		if len(objects) == 0 {
...
func merkleHistoryLeafHash(index instacoin.ChainIndex) instacoin.Hash256 {
...
func (ha *HistoryAccumulator) AppendLeaf(index instacoin.ChainIndex) {
