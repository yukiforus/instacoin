package consensus

import (
	"crypto/rand"
	"encoding/binary"
	"testing"
	"time"

	"go.instapay.kr/instacoin"
)

// copied from testutil (can't import due to cycle)
func findBlockNonce(h *instacoin.BlockHeader, target instacoin.BlockID) {
	rand.Read(h.Nonce[:])
	for !h.ID().MeetsTarget(target) {
		binary.LittleEndian.PutUint64(h.Nonce[:], binary.LittleEndian.Uint64(h.Nonce[:])+1)
	}
}

func TestScratchChain(t *testing.T) {
	pubkey, privkey := testingKeypair()
	ourAddr := pubkey.Address()

	b := genesisWithBeneficiaries([]instacoin.Beneficiary{
		{Value: instacoin.BaseUnitsPerCoin.Mul64(1), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(2), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(3), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(4), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(5), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(6), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(7), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(8), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(9), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(10), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(11), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(12), Address: ourAddr},
		{Value: instacoin.BaseUnitsPerCoin.Mul64(13), Address: ourAddr},
	}...)
	sau := GenesisUpdate(b, testingDifficulty)
	mineBlock := func(txns ...instacoin.Transaction) instacoin.Block {
		b := instacoin.Block{
			Header: instacoin.BlockHeader{
				Height:       b.Header.Height + 1,
				ParentID:     b.Header.ID(),
				Timestamp:    b.Header.Timestamp.Add(time.Second),
				MinerAddress: ourAddr,
			},
			Transactions: txns,
		}
		b.Header.Commitment = sau.Context.Commitment(b.Header.MinerAddress, b.Transactions)
		findBlockNonce(&b.Header, instacoin.HashRequiringWork(sau.Context.Difficulty))
		return b
	}

	sc := NewScratchChain(sau.Context)
	var blocks []instacoin.Block
	origOutputs := sau.NewOutputs
	toSpend := origOutputs[5:10]
	var spendTotal instacoin.Currency
	for _, o := range toSpend {
		spendTotal = spendTotal.Add(o.Value)
	}
	txn := instacoin.Transaction{
		Outputs: []instacoin.Beneficiary{{
			Value:   spendTotal.Sub(instacoin.BaseUnitsPerCoin),
			Address: ourAddr,
		}},
		MinerFee: instacoin.BaseUnitsPerCoin,
	}
	for _, o := range toSpend {
		txn.Inputs = append(txn.Inputs, instacoin.Input{
			Parent:    o,
			PublicKey: pubkey,
		})
	}
	signAllInputs(&txn, sau.Context, privkey)

	b = mineBlock(txn)
	if err := sc.AppendHeader(b.Header); err != nil {
		t.Fatal(err)
	}
	blocks = append(blocks, b)

	sau = ApplyBlock(sau.Context, b)
	sau.UpdateOutputProof(&origOutputs[2])
	newOutputs := sau.NewOutputs

	txn = instacoin.Transaction{
		Inputs: []instacoin.Input{{
			Parent:    newOutputs[1],
			PublicKey: pubkey,
		}},
		Outputs: []instacoin.Beneficiary{{
			Value:   newOutputs[1].Value.Sub(instacoin.BaseUnitsPerCoin),
			Address: ourAddr,
		}},
		MinerFee: instacoin.BaseUnitsPerCoin,
	}
	signAllInputs(&txn, sau.Context, privkey)

	b = mineBlock(txn)
	if err := sc.AppendHeader(b.Header); err != nil {
		t.Fatal(err)
	}
	blocks = append(blocks, b)
	sau = ApplyBlock(sau.Context, b)
	for i := range origOutputs {
		sau.UpdateOutputProof(&origOutputs[i])
	}
	toSpend = origOutputs[2:3]
	spendTotal = instacoin.ZeroCurrency
	for _, o := range toSpend {
		spendTotal = spendTotal.Add(o.Value)
	}
	parentTxn := instacoin.Transaction{
		Inputs: []instacoin.Input{{
			Parent:    toSpend[0],
			PublicKey: pubkey,
		}},
		Outputs: []instacoin.Beneficiary{{
			Value:   spendTotal,
			Address: ourAddr,
		}},
	}
	signAllInputs(&parentTxn, sau.Context, privkey)
	childTxn := instacoin.Transaction{
		Inputs: []instacoin.Input{{
			Parent: instacoin.Output{
				ID: instacoin.OutputID{
					TransactionID: parentTxn.ID(),
					Index:         0,
				},
				Value:     spendTotal,
				Address:   ourAddr,
				LeafIndex: instacoin.EphemeralLeafIndex,
			},
			PublicKey: pubkey,
		}},
		Outputs: []instacoin.Beneficiary{{
			Value:   spendTotal.Sub(instacoin.BaseUnitsPerCoin),
			Address: ourAddr,
		}},
		MinerFee: instacoin.BaseUnitsPerCoin,
	}
	signAllInputs(&childTxn, sau.Context, privkey)

	b = mineBlock(parentTxn, childTxn)
	if err := sc.AppendHeader(b.Header); err != nil {
		t.Fatal(err)
	}
	blocks = append(blocks, b)

	// validate all blocks
	for _, b := range blocks {
		if _, err := sc.ApplyBlock(b); err != nil {
			t.Fatal(err)
		}
	}
}

func TestScratchChainDifficultyAdjustment(t *testing.T) {
	var b instacoin.Block
	b.Header.Timestamp = time.Unix(734600000, 0)
	initialDifficulty := instacoin.Work{NumHashes: [32]byte{31: 4}}
	vc := GenesisUpdate(b, initialDifficulty).Context

	// mine enough blocks to trigger adjustment
	sc := NewScratchChain(vc)
	for i := 0; i < DifficultyAdjustmentInterval; i++ {
		b.Header = instacoin.BlockHeader{
			Height:    b.Header.Height + 1,
			ParentID:  b.Header.ID(),
			Timestamp: b.Header.Timestamp.Add(time.Second),
		}
		b.Header.Commitment = vc.Commitment(instacoin.VoidAddress, b.Transactions)
		findBlockNonce(&b.Header, instacoin.HashRequiringWork(vc.Difficulty))
		if err := sc.AppendHeader(b.Header); err != nil {
			t.Fatal(err)
		} else if _, err := sc.ApplyBlock(b); err != nil {
			t.Fatal(err)
		}
		vc = ApplyBlock(vc, b).Context
	}

	// difficulty should have changed
	currentDifficulty := sc.tvc.Difficulty
	if currentDifficulty.Cmp(initialDifficulty) <= 0 {
		t.Fatal("difficulty should have increased")
	}

	// mine a block with less than the minimum work; it should be rejected
	b.Header = instacoin.BlockHeader{
		Height:    b.Header.Height + 1,
		ParentID:  b.Header.ID(),
		Timestamp: b.Header.Timestamp.Add(time.Second),
	}
	b.Header.Commitment = vc.Commitment(instacoin.VoidAddress, b.Transactions)
	for instacoin.WorkRequiredForHash(b.ID()).Cmp(currentDifficulty) >= 0 {
		rand.Read(b.Header.Nonce[:])
	}
	if err := sc.AppendHeader(b.Header); err == nil {
		t.Fatal("expected block to be rejected")
	}
	vc = ApplyBlock(vc, b).Context

	// mine at actual difficulty
	findBlockNonce(&b.Header, instacoin.HashRequiringWork(currentDifficulty))
	if err := sc.AppendHeader(b.Header); err != nil {
		t.Fatal(err)
	} else if _, err := sc.ApplyBlock(b); err != nil {
		t.Fatal(err)
	}
	vc = ApplyBlock(vc, b).Context
}

func TestAdjustDifficulty(t *testing.T) {
	w := instacoin.Work{NumHashes: [32]byte{31: 100}}
	twice := adjustDifficulty(w, BlockInterval*DifficultyAdjustmentInterval/2)
	if twice.String() != "200" {
		t.Errorf("expected 200, got %v", twice.String())
	}
	half := adjustDifficulty(w, BlockInterval*DifficultyAdjustmentInterval*2)
	if half.String() != "50" {
		t.Errorf("expected 50, got %v", half.String())
	}
	third := adjustDifficulty(w, BlockInterval*DifficultyAdjustmentInterval*3)
	if third.String() != "33" {
		t.Errorf("expected 33, got %v", third.String())
	}
	max := adjustDifficulty(w, BlockInterval*DifficultyAdjustmentInterval/100)
	if max.String() != "400" {
		t.Errorf("expected 400, got %v", max.String())
	}
	min := adjustDifficulty(w, BlockInterval*DifficultyAdjustmentInterval*100)
	if min.String() != "25" {
		t.Errorf("expected 25, got %v", min.String())
	}
}
