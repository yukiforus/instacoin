...
contract('EIP20', (accounts) => {
  beforeEach(async () => {
    HST = await EIP20Abstraction.new(10000, 'instaCoin', 1, 'INC', { from: accounts[0] });
  });
  ...
  it('creation: test correct setting of vanity information', async () => {
    ...
    assert.strictEqual(name, 'instaCoin');

    const decimals = await HST.decimals.call();
    assert.strictEqual(decimals.toNumber(), 1);

    const symbol = await HST.symbol.call();
    assert.strictEqual(symbol, 'INC');
  });

  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const HST2 = await EIP20Abstraction.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'instaCoin', 1, 'INC', { from: accounts[0] });
