...
type EphemeralStore struct {
	entries map[instacoin.ChainIndex]consensus.Checkpoint
	best    []instacoin.ChainIndex
}
...
func (es *EphemeralStore) Checkpoint(index instacoin.ChainIndex) (consensus.Checkpoint, error) {
...
func (es *EphemeralStore) Header(index instacoin.ChainIndex) (instacoin.BlockHeader, error) {
	c, err := es.Checkpoint(index)
	return c.Block.Header, err
}

// ExtendBest implements chain.ManagerStore.
func (es *EphemeralStore) ExtendBest(index instacoin.ChainIndex) error {
...
func (es *EphemeralStore) BestIndex(height uint64) (instacoin.ChainIndex, error) {
	baseHeight, tipHeight := es.best[0].Height, es.best[len(es.best)-1].Height
	if !(baseHeight <= height && height <= tipHeight) {
		return instacoin.ChainIndex{}, chain.ErrUnknownIndex
	}
...
func NewEphemeralStore(c consensus.Checkpoint) *EphemeralStore {
	return &EphemeralStore{
		entries: map[instacoin.ChainIndex]consensus.Checkpoint{c.Context.Index: c},
		best:    []instacoin.ChainIndex{c.Context.Index},
	}
}
...
type FlatStore struct {
	...
	base    instacoin.ChainIndex
	offsets map[instacoin.ChainIndex]int64
}
...
// Checkpoint implements chain.ManagerStore.
func (fs *FlatStore) Checkpoint(index instacoin.ChainIndex) (c consensus.Checkpoint, err error) {
...
// Header implements chain.ManagerStore.
func (fs *FlatStore) Header(index instacoin.ChainIndex) (instacoin.BlockHeader, error) {
	b := make([]byte, 8+32+8+8+32+32)
	if offset, ok := fs.offsets[index]; !ok {
		return instacoin.BlockHeader{}, chain.ErrUnknownIndex
	} else if _, err := fs.entryFile.ReadAt(b, offset); err != nil {
		return instacoin.BlockHeader{}, err
	}
...
// ExtendBest implements chain.ManagerStore.
func (fs *FlatStore) ExtendBest(index instacoin.ChainIndex) error {
...
// BestIndex implements chain.ManagerStore.
func (fs *FlatStore) BestIndex(height uint64) (index instacoin.ChainIndex, err error) {
	if height < fs.base.Height {
		return instacoin.ChainIndex{}, chain.ErrPruned
	}
...
func (fs *FlatStore) recoverBest(tip instacoin.ChainIndex) error {
...
func NewFlatStore(dir string, c consensus.Checkpoint) (*FlatStore, consensus.Checkpoint, error) {
	...
	offsets := make(map[instacoin.ChainIndex]int64)
	for {
...
func writeBest(w io.Writer, index instacoin.ChainIndex) error {
	buf := make([]byte, bestSize)
...
func readBest(r io.Reader) (index instacoin.ChainIndex, err error) {
	buf := make([]byte, bestSize)
	if _, err = io.ReadFull(r, buf); err != nil {
		return
	}
...
func writeIndex(w io.Writer, index instacoin.ChainIndex, offset int64) error {
...
func readIndex(r io.Reader) (index instacoin.ChainIndex, offset int64, err error) {
	buf := make([]byte, indexSize)
	if _, err = io.ReadFull(r, buf); err != nil {
		return
	}
...
func writeCheckpoint(w io.Writer, c consensus.Checkpoint) error {
	...
	writeCurrency := func(c instacoin.Currency) {
		writeUint64(c.Lo)
		writeUint64(c.Hi)
	}
	writeOutputID := func(id instacoin.OutputID) {
...
func readCheckpoint(r io.Reader, c *consensus.Checkpoint) error {
	...
	readCurrency := func() (c instacoin.Currency) {
		return instacoin.NewCurrency(readUint64(), readUint64())
	}
	readOutputID := func() instacoin.OutputID {
		return instacoin.OutputID{
	...
	// read txns
	c.Block.Transactions = make([]instacoin.Transaction, readUint64())
	for i := range c.Block.Transactions {
		txn := &c.Block.Transactions[i]
		txn.Inputs = make([]instacoin.Input, readUint64())
		for j := range txn.Inputs {
			in := &txn.Inputs[j]
			...
			in.Parent.MerkleProof = make([]instacoin.Hash256, readUint64())
			in.Parent.LeafIndex = readUint64()
			in.PublicKey = readHash()
			read(in.Signature[:])
		}
		txn.Outputs = make([]instacoin.Beneficiary, readUint64())
		for j := range txn.Outputs {
			out := &txn.Outputs[j]
