...
type EphemeralStore struct {
	mu        sync.Mutex
	addrs     map[instacoin.Address]uint64
...
func (s *EphemeralStore) AddAddress(addr instacoin.Address, index uint64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.addrs[addr] = index
...
func (s *EphemeralStore) AddressIndex(addr instacoin.Address) (uint64, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *EphemeralStore) SpendableOutputs() []instacoin.Output {
	s.mu.Lock()
	defer s.mu.Unlock()
	var spendable []instacoin.Output
	for _, o := range s.outputs {
		if o.Timelock <= s.vc.Index.Height {
			o.MerkleProof = append([]instacoin.Hash256(nil), o.MerkleProof...)
...
func (s *EphemeralStore) ProcessChainApplyUpdate(cau *chain.ApplyUpdate, _ bool) error {
	...
	for _, txn := range cau.Block.Transactions {
		...
		var inflow, outflow instacoin.Currency
...
func NewEphemeralStore() *EphemeralStore {
	return &EphemeralStore{
		addrs: make(map[instacoin.Address]uint64),
	}
}
...
func (s *JSONStore) load(vc consensus.ValidationContext) (instacoin.ChainIndex, error) {
	var p persistData
	if js, err := os.ReadFile(filepath.Join(s.dir, "wallet.json")); os.IsNotExist(err) {
		// set defaults
		s.addrs = make(map[instacoin.Address]uint64)
		s.vc = vc
		return vc.Index, nil
	} else if err != nil {
		return instacoin.ChainIndex{}, err
	} else if err := json.Unmarshal(js, &p); err != nil {
		return instacoin.ChainIndex{}, err
	}
	s.addrs = make(map[instacoin.Address]uint64, len(p.Addrs))
	for k, v := range p.Addrs {
		var addr instacoin.Address
...
// AddAddress implements Store.
func (s *JSONStore) AddAddress(addr instacoin.Address, index uint64) error {
...
func NewJSONStore(dir string, vc consensus.ValidationContext) (*JSONStore, instacoin.ChainIndex, error) {
	s := &JSONStore{
	...
	tip, err := s.load(vc)
	if err != nil {
		return nil, instacoin.ChainIndex{}, err
	}
	return s, tip, nil
}
