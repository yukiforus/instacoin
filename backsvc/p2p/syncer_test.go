...
func TestMsgGetHeaders(t *testing.T) {
	...
	tests := []struct {
		...
		{
			&MsgGetHeaders{History: []instacoin.ChainIndex{
				{Height: 42, ID: instacoin.BlockID{1, 2, 3}},
				{Height: 14, ID: instacoin.BlockID{4, 5, 6}},
				{Height: 99, ID: instacoin.BlockID{9, 9, 9}},
			}},
			&MsgHeaders{Headers: headers},
		},
		// just tip; should reply with nothing
		{
			...
			&MsgHeaders{Headers: []instacoin.BlockHeader{}},
		},
...
func TestMsgGetBlocks(t *testing.T) {
	...
	exp = &MsgBlocks{Blocks: []instacoin.Block{b12.Block}}
	if !reflect.DeepEqual(outbox, exp) {
		t.Errorf("\nexpected:\n\t%v\ngot:\n\t%v\n", exp, outbox)
	}
}
