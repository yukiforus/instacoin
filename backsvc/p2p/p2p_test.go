...
func (tn *testNode) send(amount instacoin.Currency, dest instacoin.Address) error {
	...
	tn.s.Broadcast(&MsgRelayTransactionSet{Transactions: []instacoin.Transaction{txn}})
	return nil
}
...
func newTestNode(tb testing.TB, genesisID instacoin.BlockID, c consensus.Checkpoint) *testNode {
...
func TestNetwork(t *testing.T) {
	...
	sau := consensus.GenesisUpdate(genesisBlock, instacoin.Work{NumHashes: [32]byte{30: 1 << 2}})
	...
	for n1.c.Tip().Height < 100 {
		...
		go n1.send(instacoin.BaseUnitsPerCoin.Mul64(7), n2addr)
		go n2.send(instacoin.BaseUnitsPerCoin.Mul64(9), n1addr)
	}
	n1.stopMining()
	n2.stopMining()
